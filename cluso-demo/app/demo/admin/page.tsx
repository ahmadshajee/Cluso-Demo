...
export default function AdminDashboardPage() {
  ...
  const cards = [
    { label: "Pending Requests", value: pendingCount, icon: Clock, tone: "amber" as const },
    { label: "Approved Requests", value: approvedCount, icon: CheckCircle2, tone: "emerald" as const },
    { label: "Verified Requests", value: verifiedCount, icon: Shield, tone: "cyan" as const },
    { label: "Rejected Requests", value: rejectedCount, icon: AlertTriangle, tone: "rose" as const },
    { label: "Archived Requests", value: 2, icon: Archive, tone: "violet" as const },
    { label: "Services", value: 6, icon: Package, tone: "sky" as const },
    { label: "Companies", value: 3, icon: Building, tone: "cyan" as const },
    { label: "Verifiers", value: 4, icon: Users, tone: "sky" as const },
    { label: "Admins", value: 2, icon: Shield, tone: "violet" as const },
  ];
      ...
      >
        <section className="portal-stats-grid" aria-label="Admin overview metrics">
          {cards.map((item) => {
            ...
            return (
              <div key={item.label} className={`portal-stat portal-stat-link portal-stat-${item.tone}`}>
